...
function matchWithArgMap(patternPart, triplePart, context) {
  ...
  if (inList && inList.includes(triplePart) || $inList && $inList.includes(triplePart)) {
    return context;
  }
  if (patternPart.hasOwnProperty("$not") || patternPart.hasOwnProperty("$isNull")) {
    return context;
  }
  return null;
}
...
function valueAttrPat(makeVar, store, valueEtype, valueLevel, valueLabel, v) {
  const attr = getAttrByFwdIdentName(store, valueEtype, valueLabel);
  if (!attr) {
...
function runDataloadAndReturnObjects(store, etype, direction, pageInfo, dq) {
  ...
  const idVecs = query(store, dq).sort(function sortIdVecs([_, tsA], [__, tsB]) {
    ...
    for (const { etype, lookupPair, linkLabel } of lookupPairsOfOp(op)) {
      ...
      if (linkLabel) {
        ...
        if (isRefLookupIdent(attrs, linkEtype, identName)) {
          ...
          if (!attr) {
            addAttr2(createObjectAttr(linkEtype, identName, lookupProps));
          }
          addUnsynced(attr);
        }
    ...
    if (OBJ_ACTIONS.has(action)) {
      ...
      for (const label of labels) {
        ...
        if (UPDATE_ACTIONS.has(action)) {
          if (!fwdAttr) {
            addAttr2(createObjectAttr(etype, label, label === "id" ? { "unique?": true } : null));
          }
        }
...
var DoNotUseInstantSchema = class {
...
function do_not_use_schema({ entities, links, rooms }) {
...
function _init_internal(config, Storage2, NetworkListener, versions) {
  const existingClient = globalInstantCoreStore[config.appId];
  if (existingClient) {
    return existingClient;
  }
  const reactor = new Reactor(Object.assign(Object.assign({}, defaultConfig2), config), Storage2 || IndexedDBStorage, NetworkListener || WindowNetworkListener, versions);
...
function _do_not_use_init_internal(config, Storage2, NetworkListener) {
  const existingClient = globalInstantCoreStore[config.appId];
  if (existingClient) {
    return existingClient;
  }
  const reactor = new Reactor(Object.assign(Object.assign(Object.assign({}, defaultConfig2), config), { cardinalityInference: config.schema ? true : false }), Storage2 || IndexedDBStorage, NetworkListener || WindowNetworkListener);
  const client = new DoNotUseInstantCore(reactor);
  globalInstantCoreStore[config.appId] = client;
  if (typeof window !== "undefined" && typeof window.location !== "undefined") {
    const showDevtool = (
      // show widget by default?
...
var InstantReact = class {
  constructor(config, versions) {
    ...
    this._core = _init_internal(
      ...
      Object.assign(Object.assign({}, versions || {}), { "@instantdb/react": version_default3 })
    );
...
// node_modules/@instantdb/react/dist/module/DoNotUseInstantReact.js
var import_react4 = __toESM(require_react());
var defaultActivityStopTimeout2 = 1e3;
var DoNotUseInstantReactRoom = class {
  constructor(_core, type, id2) {
    this.useTopicEffect = (topic, onEvent) => {
    ...
    this.usePublishTopic = (topic) => {
      (0, import_react4.useEffect)(() => this._core._reactor.joinRoom(this.id), [this.id]);
      const publishTopic = (0, import_react4.useCallback)((data) => {
        this._core._reactor.publishTopic({
          roomType: this.type,
          roomId: this.id,
          topic,
    ...
    this.useTypingIndicator = (inputName, opts = {}) => {
      ...
      const setActive = (isActive) => {
        ...
        if ((opts === null || opts === void 0 ? void 0 : opts.timeout) === null || (opts === null || opts === void 0 ? void 0 : opts.timeout) === 0)
          return;
        timeout.set((_a = opts === null || opts === void 0 ? void 0 : opts.timeout) !== null && _a !== void 0 ? _a : defaultActivityStopTimeout2, () => {
          this._core._reactor.publishPresence(this.type, this.id, {
            [inputName]: null
          });
        });
...
var defaultAuthState2 = {
  isLoading: true,
  user: void 0,
  error: void 0
};
var DoNotUseInstantReact = class {
  constructor(config) {
    this.tx = txInit();
    this.getLocalId = (name) => {
      return this._core.getLocalId(name);
    ...
    this.useQuery = (query3) => {
      return doNotUseUseQuery(this._core, query3).state;
    };
    this.useAuth = () => {
      const resultCacheRef = (0, import_react4.useRef)(this._core._reactor._currentUserCached);
      ...
      const state = (0, import_react4.useSyncExternalStore)(subscribe, () => resultCacheRef.current, () => defaultAuthState2);
      return state;
    };
    this.queryOnce = (query3) => {
      return this._core.queryOnce(query3);
    };
    this._core = _do_not_use_init_internal(
      config,
      // @ts-expect-error because TS can't resolve subclass statics
      this.constructor.Storage,
      // @ts-expect-error because TS can't resolve subclass statics
   ...
   */
  room(type = "_defaultRoomType", id2 = "_defaultRoomId") {
    return new DoNotUseInstantReactRoom(this._core, type, id2);
  }
};
...
// node_modules/@instantdb/react/dist/module/DoNotUseInstantReactWeb.js
var DoNotUseInstantReactWeb = class extends DoNotUseInstantReact {
};
