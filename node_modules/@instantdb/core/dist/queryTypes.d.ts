import type { EntitiesDef, IInstantDataSchema, InstantGraph, LinkAttrDef, ResolveAttrs, ResolveEntityAttrs } from "./schemaTypes";
type NonEmpty<T> = {
...
type DoNotUseQueryResponse<Q, Schema> = Schema extends IInstantDataSchema<any, any> ? DoNotUseInstaQLQueryResult<Schema, Q> : never;
...
type DoNotUseInstaQLSubqueryResult<Schema extends IInstantDataSchema<EntitiesDef, any>, EntityName extends keyof Schema["entities"], Query extends {
    [LinkAttrName in keyof Schema["entities"][EntityName]["links"]]?: any;
}> = {
    [QueryPropName in keyof Query]: Schema["entities"][EntityName]["links"][QueryPropName] extends LinkAttrDef<infer Cardinality, infer LinkedEntityName> ? LinkedEntityName extends keyof Schema["entities"] ? Cardinality extends "one" ? DoNotUseInstantEntity<Schema, LinkedEntityName, Query[QueryPropName]> | undefined : DoNotUseInstantEntity<Schema, LinkedEntityName, Query[QueryPropName]>[] : never : never;
};
...
type DoNotUseInstantEntity<Schema extends IInstantDataSchema<EntitiesDef, any>, EntityName extends keyof Schema["entities"], Subquery extends {
    [QueryPropName in keyof Schema["entities"][EntityName]["links"]]?: any;
} = {}> = ResolveEntityAttrs<Schema["entities"][EntityName]> & DoNotUseInstaQLSubqueryResult<Schema, EntityName, Subquery>;
...
type DoNotUseInstaQLQueryResult<Schema extends IInstantDataSchema<EntitiesDef, any>, Query> = {
    [QueryPropName in keyof Query]: QueryPropName extends keyof Schema["entities"] ? DoNotUseInstantEntity<Schema, QueryPropName, Query[QueryPropName]>[] : never;
};
type InstaQLQuerySubqueryParams<S extends IInstantDataSchema<any, any>, E extends keyof S["entities"]> = {
...
export { Query, QueryResponse, DoNotUseQueryResponse, PageInfoResponse, InstantObject, Exactly, Remove$, InstaQLQueryResult, InstaQLQueryParams, InstaQLQueryEntityResult, DoNotUseInstantEntity, DoNotUseInstaQLQueryResult, Cursor, };
